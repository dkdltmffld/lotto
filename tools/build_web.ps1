# 웹(HTML5) 빌드 → build/web/index.html
#
# 사용:
...
#   "File not found" 에러만). 외부 공개용 빌드는 에디터를 닫고 빌드해야 mcp 재주입을 막을 수 있음.
...
param(
    ...
    [string]$Preset = "Web",
    [switch]$Release   # release export로 빌드. 기본=debug export — 치트 게이트가 로컬 웹 테스트에서 살아있게 함.
)
...
# 빌드 flavor 마커 — 디버그/release 산출물 구분용.
